#!/usr/bin/env python3
"""
step2_database_schema.py

Creates:
  - docker-compose.yml (Postgres for local dev)
  - backend/package.json, backend/tsconfig.json
  - backend/drizzle.config.ts
  - backend/src/db/schema.ts   (full Drizzle schema)
  - backend/src/db/client.ts

Safe to re-run: will not overwrite files that already have content.

Usage:
    cd /Users/macbook/verdict
    python3 scripts/step2_database_schema.py
"""
...
services:
  postgres:
    ...
    container_name: verdict_postgres
    restart: unless-stopped
    environment:
      POSTGRES_USER: ${POSTGRES_USER:-verdict_app}
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:-changeme_local_dev_only}
      POSTGRES_DB: ${POSTGRES_DB:-verdict}
    ports:
      - "${POSTGRES_PORT:-5432}:5432"
    volumes:
      - verdict_pgdata:/var/lib/postgresql/data
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U ${POSTGRES_USER:-verdict_app} -d ${POSTGRES_DB:-verdict}"]
      interval: 5s
      timeout: 5s
      retries: 10
...
BACKEND_PACKAGE_JSON = """\
{
  ...
  "private": true,
  "type": "module",
  "description": "VERDICT backend API: auth, case/evidence indexing, GenLayer event sync, evidence file storage.",
  "scripts": {
...
def main() -> None:
    print(f"Step 2: Database schema — project root: {PROJECT_ROOT}\n")
    write_if_missing(PROJECT_ROOT / "docker-compose.yml", DOCKER_COMPOSE)
    write_if_missing(PROJECT_ROOT / "backend" / "package.json", BACKEND_PACKAGE_JSON)
    write_if_missing(PROJECT_ROOT / "backend" / "tsconfig.json", BACKEND_TSCONFIG)
    write_if_missing(PROJECT_ROOT / "backend" / "drizzle.config.ts", DRIZZLE_CONFIG)
    write_if_missing(PROJECT_ROOT / "backend" / "src" / "db" / "client.ts", DB_CLIENT_TS)
    write_if_missing(PROJECT_ROOT / "backend" / "src" / "db" / "schema.ts", DB_SCHEMA_TS)
    write_if_missing(PROJECT_ROOT / "backend" / "src" / "db" / "migrate.ts", DB_MIGRATE_TS)
    print("\nDone.")
